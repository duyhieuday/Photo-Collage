...
# Supports rotated cells: @(x0,y0,x1,y1,angleDeg). GDI RotateTransform == Android canvas.rotate.
...
foreach($id in @($CELLS.Keys)){
  ...
  foreach($c in $rr){
    ...
    if($c.Count -ge 5 -and ([double]$c[4]) -ne 0){
      ...
      $gr.RotateTransform([float]$c[4])
      $gr.DrawRectangle($pen,[float](-$rw/2),[float](-$rh/2),[float]$rw,[float]$rh)
      $gr.Restore($st)
    } else {
