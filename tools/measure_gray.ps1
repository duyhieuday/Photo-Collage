# Find largest near-gray rectangular openings in temp_<id>.jpg (logic 1125x2000). ASCII only.
Add-Type -AssemblyName System.Drawing
$id=$args[0]
$lo=200; $hi=245
if($args.Count -ge 2){ $lo=[int]$args[1] }
if($args.Count -ge 3){ $hi=[int]$args[2] }
...
for($ry=0;$ry -lt $rows;$ry++){
  for($rx=0;$rx -lt $cols;$rx++){
    ...
    if($px.R -ge $lo -and $px.R -le $hi -and ($mx-$mn) -le 8){ $mask[$ry,$rx]=$true }
  }
}
